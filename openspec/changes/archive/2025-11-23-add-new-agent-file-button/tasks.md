...
- [x] Register `openspec-for-agent.prompts.createAgentFile` command in `package.json` <!-- id: 0 -->
- [x] Add "New Agent File" menu item to `view/title` for `openspec-for-agent.views.promptsExplorer` in `package.json` with `group: "navigation@1"` <!-- id: 1 -->
...
- [x] Implement `openspec-for-agent.prompts.createAgentFile` command handler in `src/extension.ts` to execute `workbench.command.new.agent` <!-- id: 5 -->
