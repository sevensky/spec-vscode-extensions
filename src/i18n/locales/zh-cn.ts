...
export const zhCn: Record<string, string> = {
	...
	"prompt.deprecationWarning": "提示词文件 '{legacy}' 已重命名为 '{current}'，请更新引用。",
	"prompt.migrationError": "未找到 OpenSpec v1 提示词文件（当前 agent：{agent}）。\n\n需要：{requiredPath}\n\n可能原因与解决办法：\n\n1. 尚未初始化 OpenSpec v1：\n   npm install -g openspec@latest\n   cd {workspace}\n   openspec init\n\n2. 全局配置 delivery 设为 \"skills\" 导致命令文件未生成：\n   检查 ~/.config/openspec/config.json，将 \"delivery\" 改为 \"both\" 后重新运行：\n   openspec init --force\n\n3. 工作流配置未包含所需工作流：\n   确认 config.json 的 workflows 包含 \"propose\"（生成 propose 命令文件）。\n\n更多信息请参见工作区中的 README.md。",
	"prompt.legacyDeprecation": "⚠️ 正在使用旧版 OpenSpec v0.x 提示词文件：{legacy}\n\n请通过在工作区运行 'openspec init' 迁移到 OpenSpec v1。\n这将创建新的提示词文件：{current}\n\n旧版支持将在未来版本中移除。",
	"prompt.templatePlaceholder": "在此描述你的提示词。执行时此文件内容将发送给 Copilot。",
	...
	"steering.globalExists": "全局配置文件（~/.github/copilot-instructions.md）已存在。是否覆盖？",
	"steering.agentsExists": "项目 AGENTS.md（openspec/AGENTS.md）已存在。是否覆盖？",
	...
	// ── claude service ──
	"claude.notInstalled": "Claude Code CLI（\"claude\"）未安装或不在 PATH 中。请安装 Claude Code 后重试，或在设置中将 aiAgent 切换回 github-copilot / codex。",
